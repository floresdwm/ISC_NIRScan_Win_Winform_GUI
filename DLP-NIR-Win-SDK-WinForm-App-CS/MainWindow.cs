...
namespace DLP_NIR_Win_SDK_WinForm_App_CS
{
    public partial class MainWindow : Form
    {
        ...
        {
            ProgressWindowStart("Device Open", "Connecting " + ModelnSN + "... Please wait!", false);
        }
            ...
            if (senderName == "TextBox")
            {
                ...
                else if (cfgFieldName.Contains("CfgRangeStart"))
                {
                    ...
                    if (!long.TryParse(cfgField.Text, out rangeStart))
                    {
                        ...
                        cfgField.Text = MIN_WAVELENGTH.ToString();
                        cfgField.Focus();
                    }
                    else if (rangeStart > 1650)
                    {
                        String errMsg = "Invalid input! Wavelength start should be less than " + (MAX_WAVELENGTH - 50).ToString();
                        Message.ShowError(errMsg);
                        cfgField.Text = (MAX_WAVELENGTH - 50).ToString();
                        cfgField.Focus();
                    }
                    else if (rangeStart < MIN_WAVELENGTH)
                    {
                        String errMsg = "Invalid input! Wavelength start should be greater than " + MIN_WAVELENGTH.ToString();
                        Message.ShowError(errMsg);
                        cfgField.Text = MIN_WAVELENGTH.ToString();
                        cfgField.Focus();
                    }
                ...
                else if (cfgFieldName.Contains("CfgRangeEnd"))
                {
                    ...
                    if (!long.TryParse(cfgField.Text, out rangeEnd))
                    {
                        ...
                        cfgField.Text = MAX_WAVELENGTH.ToString();
                        cfgField.Focus();
                    }
                    else if (rangeEnd > MAX_WAVELENGTH)
                    {
                        String errMsg = "Invalid input! Wavelength end should be less than " + MAX_WAVELENGTH.ToString();
                        Message.ShowError(errMsg);
                        cfgField.Text = MAX_WAVELENGTH.ToString();
                        cfgField.Focus();
                    }
                    else if (rangeEnd < 950)
                    {
                        String errMsg = "Invalid input! Wavelength end should be greater than " + (MIN_WAVELENGTH + 50).ToString();
                        Message.ShowError(errMsg);
                        cfgField.Text = (MIN_WAVELENGTH + 50).ToString();
                        cfgField.Focus();
                    }
                }
        ...
        private int Update_Scan_Resolution_and_Pattern_Label()
        {
            ...
            for (int i = 0; i < num_sections; i++)
            {
                ...
                if (rangeStart >= MIN_WAVELENGTH && rangeEnd <= MAX_WAVELENGTH)
                {
                    ...
                    if (numPat > MaxResolution)
                    {
                        ...
                        Message.ShowError("Exceed max resolution!");
        ...
        private void SaveHeader_CSV(StreamWriter sw)
        {
            ...
            for (int i = 0; i < 2; i++)
            {
                ...
                CSV[12, i * 7] = "Humidity (%):,";
                if (MB_Ver >= 'F' && i == 0)//only scan save lamp ADC
                {
                    CSV[13, i * 7] = "Lamp ADC(VM1/VM2/CM1/CM2):,";
                }
                else
            ...
            for (int i = 0; i < Scan.ScanConfigData.head.num_sections; i++)
            {
                if (i == 0)
                {
                    ...
                    if (MB_Ver >= 'F')
                    {
                        if (Device.ReadLampParam() == 0)
                        {
                            CSV[13, 1] = Device.LampADC[0] + ",";//VM1
                            CSV[13, 2] = Device.LampADC[1] + ",";//VM2
                            CSV[13, 3] = Device.LampADC[2] + ",";//CM1
                            CSV[13, 4] = Device.LampADC[3] + ",";//CM2
                        }
                        else
                        {
                            CSV[13, 1] = "NA,";//VM1
        ...
        {
            Button_CfgCancel_Click(this, null);
            if (Device.IsConnected())
            {
        ...
        private void Button_Get_BLE_Display_Name_Click(object sender, EventArgs e)
        {
            ...
            if (Device.ReadBleDispName(pOutBuf) == SDK.RETURN_PASS)
        ...
        private Int32 SaveCfgToLocalOrDevice(Boolean IsTarget)
        {
            ...
            else
            {
                ...
                //String FileNameWithSuffix = String.Join("-",Device.DevInfo.DeviceUUID);
                String FileNameWithSuffix = BitConverter.ToString(Device.DevInfo.DeviceUUID).Replace("-", "");
                FileNameWithSuffix = "ConfigList_" + FileNameWithSuffix + ".xml";
                String FileName = Path.Combine(ConfigDir, FileNameWithSuffix);
                XmlSerializer xml = new XmlSerializer(typeof(List<ScanConfig.SlewScanConfig>));
        ...
        {
            t_PBW = new Thread(() => ProgressWindowShow(title, content, cancellable));
            t_PBW.Start();
        }

        private void ProgressWindowShow(String title, String content, Boolean cancellable)
        {
